...
def github_login(request):
    ...
    redirect_uri = "http://airbnb-clone.eba-rja8a647.ap-northeast-2.elasticbeanstalk.com/users/login/github/callback"
    return redirect(
...
def kakao_login(request):
    ...
    redirect_uri = "http://airbnb-clone.eba-rja8a647.ap-northeast-2.elasticbeanstalk.com/users/login/kakao/callback"
    return redirect(
        f"https://kauth.kakao.com/oauth/authorize?client_id={client_id}&redirect_uri={redirect_uri}&response_type=code"
    )  # 끝에 HTTP/1.1 없애니까 오류 안 뜸.
...
def kakao_callback(request):
    try:
        ...
        profile_request = requests.get(
            "https://kapi.kakao.com/v2/user/me",  # 2019/11/26 기준으로 v2로 바뀜. 강의댓글 참고
...
def switch_hosting(request):
    # request.session.pop("is_hosting", True) 이 방법도 있음.
    try:
